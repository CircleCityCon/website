...
class Talk:
    def __init__(self, row, talk_type='talk'):
        self.title = row.get('\ufefftitle')
        self.description = soup(
            row.get('description'), features='html.parser').text
        self.type = talk_type
...
@click.command()
@click.option('--report', '-r', type=click.File(), prompt='Report CSV File',
    help='The CSV report to process')
...
def process(report, overwrite, avatars_path, bios_path, talks_path, workshops_path):
    '''
